...
### 14.5 macOS 双轨采集实测（2026-09-11，MacBook Pro / macOS 26.5.2 / arm64）

采集层已扩到 macOS，走 cpal 0.18.2 的 CoreAudio process tap（详见
`docs/superpowers/specs/2026-09-11-macos-capture-design.md`）。

| 项 | 实测 | 目标 |
| :--- | :--- | :--- |
| 双轨时长对齐误差（稳定态） | **23ms**（9.751s / 9.774s） | §8.1 <200ms ✓ |
| 双轨时长对齐误差（首次录制） | **3178ms**（8.570s / 11.748s） | ✗ 见下 |
| 麦克风轨峰值 | 1072 / 32768 | 非零即可 |
| 系统轨峰值 | 8382 / 32768 | 非零即可 |

两条值得记录的结论：

1. **首次访问麦克风有约 3 秒的一次性开销。** 第一次录制时系统要做麦克风权限检查，
   期间麦克风轨拿不到数据，导致两轨差了 3.18 秒——远超 §8.1 的 200ms。第二次起稳定在 25ms 内。
   这对「点开始就录」的会议场景是个真实隐患，**v1.0 应在应用启动时预热一次麦克风流**把它摊掉。
2. **没有音频播放时，CoreAudio 的 tap 根本不触发回调**，系统轨连一个分片都不会生成，
   且不抛任何错误。这与最初的设想（生成一堆全零分片）不同，静音判定必须把「零分片」也算进去，
   否则权限缺失时用户只会拿到一场空录音而毫无提示。

> 仍未测：长会议（>1 小时）下的稳定性、设备热插拔、蓝牙耳机切换。
